...
export default function AddAndEditProject({
  ...
  const [imageState, setImageState] = useState<(string | File)[]>(projectState.gallery)
  ...
  async function handleAddProjects(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (editState) {
        const fileToDelete = projectState.gallery.filter((img) => !imageState.includes(img))
    ...
    >
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="max-h-[70vh] overflow-y-auto pr-2">
          <form className="scrollbar-thin space-y-4" onSubmit={handleAddProjects}>
            <Input
            ...
            <ImageSelector imageState={imageState} setImageState={setImageState} />
            <TagInput
            ...
            <div className="flex flex-wrap gap-2">
              {skills
                .filter((skill) => !projectState.technologies.includes(skill.title))
                .map((skill, index) => (
                  <Badge
                    key={index}
                    className="bg-primary-foreground text-foreground flex items-center gap-1"
                    onClick={() =>
                      setProjectState((prev) => ({
                        ...prev,
            ...
            <Button type="submit" disabled={isPending} className="bg-gradient-primary w-full">
              {isPending && (
                <>
...
export function ImageSelector({ imageState, setImageState }: ImageSelectorProps) {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      setImageState((prev) => [...prev, ...(Array.from(files) as File[])])
    }
  }
    ...
    <>
      ...
      <div className="flex cursor-pointer flex-wrap gap-2">
        {imageState.map((src, index) => (
          <div key={index} className="relative h-[100px] w-[150px] overflow-y-auto">
            <Image
            ...
            <button
              ...
              className="absolute top-1 right-1 rounded-full bg-white/70 p-1 text-red-500 hover:bg-white"
            >
              <DeleteIcon size={16} />
            </button>
          </div>
